...
// 購物車資料
const carts = [
  { id: 'cart-1', product: products[0], quantity: 2 },//prod-1
  { id: 'cart-2', product: products[2], quantity: 1 },//prod-3
  { id: 'cart-3', product: products[4], quantity: 1 }//prod-5
];
...
*/
  ...
  //寫法二：
const prod = products.find(products =>products.id === productId);
return prod ?? null;
}
//
...
function getDiscountRate(product) {
  ...
  let result = discount.toFixed(1); //將折扣率轉換為字串，並保留一位小數
  result = result[0] + result[result.length -1]; //取字串的最左邊第一位文字，跟最右邊的第一位文字
  return `${result}折`;
}
...
function calculateCartOriginalTotal(carts) {
  // 請實作此函式
  const totalOfCartsOri = carts.reduce((total, item) => total + item.product.origin_price * item.quantity, 0);
  return totalOfCartsOri;
}
...
function calculateCartTotal(carts) {
  // 請實作此函式
    const totalOfCarts = carts.reduce((total, item) => total + item.product.price * item.quantity,0);
    return totalOfCarts;
    }
...
function calculateCartItemCount(carts) {
  // 請實作此函式
  const totalOfItems = carts.reduce((total, item) =>total + item.quantity,0 );
  return totalOfItems;
}
...
function isProductInCart(carts, productId) {
  // 請實作此函式
  // //寫法一：用find找出符合條件的產品，如果找不到會回傳undefined
  // const hasProduct = carts.find(item => item.product.id === productId);
  ...
  //寫法二：用some檢查"是否"有符合條件的產品，會回傳true或false  
  // return carts.some(item => item.product.id === productId);
    const hasProduct = carts.some(item => item.product.id === productId);
    return hasProduct;
}
// ========================================
// 任務三：購物車操作模組 (進階)
...
// console.log('getProductsByCategory:', getProductsByCategory(products, '全部'));
// console.log('getDiscountRate:', getDiscountRate(products[2]));
